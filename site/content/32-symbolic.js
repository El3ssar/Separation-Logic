registerChapter({
  ...
  blocks: [
    ...
    {t:'ex',
     ...
     pitfall:"Choosing the middle assertion as <code>pure (fun σ => σ x = v) ∗ (l ↦ v)</code> — the load rule&rsquo;s postcondition, unchanged. It is the natural first guess and it makes the second half unprovable, because <code>hoare_frame</code>&rsquo;s conclusion is <code>Hoare (P ∗ R) c (Q ∗ R)</code> with the frame on the right. There is no version of the rule with the frame on the left, and Unit 27 priced the one you would have to prove instead: <code>Hoare (R ∗ P) c (R ∗ Q)</code> is just as true, and takes ten lines and four appeals to lemmas about heaps where the shipped proof needs none — after which every application inherits the swapped order. Put the commutation in your own proof instead, where it is one name.",
    ...
    {t:'ex',
     ...
     setup:"<code>copyCell tmp src dst</code> is <code>.load tmp src ;; .write dst (.var tmp)</code>. The middle assertion is the one worked out in the numbered plan above. All six names the proof needs — <code>hoare_load</code>, <code>heapLocal_load</code>, <code>preserves_of_heapOnly</code>, <code>heapOnly_pointsTo</code>, <code>hoare_write_val</code>, <code>heapLocal_write</code> — are already in scope, along with this unit&rsquo;s <code>storeStable_write</code> and <code>pure_star_regroup</code>.",
     goal:"theorem copyCell_spec (tmp : Var) (src dst : Loc) (a b : Val) :\n    Hoare ((src ↦ a) ∗ (dst ↦ b)) (copyCell tmp src dst) ((src ↦ a) ∗ (dst ↦ a)) := by",
     hints: [
       "The goal is a triple about a <code>;;</code>, so the first tactic is <code>refine hoare_seq (Q := …) ?_ ?_</code> and the whole exercise is the <code>…</code>. Written out, the two remaining goals will be a triple about <code>Cmd.load tmp src</code> and a triple about <code>Cmd.write dst (Atom.var tmp)</code>, each with the assertion you chose on one side of it.",
       "The write needs to own <code>dst</code> and to know that <code>tmp</code> holds <code>a</code>; it must not lose <code>src</code>, which is therefore its frame and goes on the right of the star. So the middle assertion is <code>(aAnd (fact (fun σ => σ tmp = a)) (dst ↦ b)) ∗ (src ↦ a)</code>. Each half of the proof is then: apply the small rule, frame it, and reshape the star until the two ends meet.",
       "First half: <code>hoare_frame (hoare_load tmp src a) (heapLocal_load tmp src) (preserves_of_heapOnly _ (heapOnly_pointsTo dst b))</code>, then <code>hoare_consequence (entails_refl _)</code> around it, leaving an entailment. Discharge that entailment with a chain of <code>entails_trans</code> over <code>star_assoc_left</code>, <code>star_mono_right</code> with <code>star_comm</code> inside it, and <code>pure_star_regroup</code>. Second half: <code>hoare_frame (hoare_write_val dst (.var tmp) b a) (heapLocal_write dst (.var tmp)) (preserves_of_storeStable (storeStable_write dst (.var tmp)) _)</code>, then one <code>hoare_consequence</code> with <code>star_comm</code>.",
    ...
    {t:'ex',
     ...
     hints: [
       "An <code>↔</code> is two implications, so the first tactic is <code>constructor</code> and there are two goals. In each, you are given a derivation whose last step is a <code>seq</code> and must produce one whose last step is a <code>seq</code> with the brackets the other way.",
     ...
     deep: [
       ...
       {t:'code', tag:'illustration', cap:'From runs to triples, and the payoff: the right-nested program, for free.',
    ...
    {t:'ex',
     ...
     hints: [
       "The program is <code>copyCell tmp src dst ;; .free src</code>, so <code>hoare_seq</code> splits it into a triple you have already proved and a triple about a single <code>free</code>. The question is what to write between them.",
     ...
     pitfall:"Choosing <code>(Q := (dst ↦ a) ∗ (src ↦ a))</code> — the cells in the order the specification&rsquo;s postcondition suggests. Then <code>copyCell_spec</code> no longer fits the first slot, since its postcondition has them the other way round, and you have bought yourself a <code>star_comm</code> in the first half to save nothing in the second. The middle assertion should be read off the neighbours, and one of the neighbours here is a theorem you already have.",
     variants:"Free <code>dst</code> instead of <code>src</code> and the specification becomes <code>Hoare ((src ↦ a) ∗ (dst ↦ b)) … (src ↦ a)</code>, provable by the same four lines with one <code>star_comm</code> added, because the cell to be freed is now on the wrong side of the star. Drop the frame and try to prove the free rule against the whole precondition and it fails at the precondition: <code>hoare_free</code> wants <code>src ↦ a</code> exactly, and <code>(src ↦ a) ∗ (dst ↦ a)</code> does not entail it — that is <code>star_not_weakening</code>, and it is the reason a frame rule was needed in the first place rather than a weakening."},
