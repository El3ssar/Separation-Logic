registerChapter({
  ...
  blocks: [
    ...
    {t:'code', tag:'illustration', cap:'Compiled. The <code>fact</code> version of the load rule is provable — the same cut, the same four lines. So the choice is not between a rule that works and one that does not.',
    ...
    {t:'code', tag:'illustration', cap:'Compiled, from two Unit 17 theorems and nothing else. The corresponding statement with <code>fact</code> in place of <code>pure</code> is the non-entailment above.',
    ...
    {t:'code', tag:'illustration', cap:'Compiled. Four lines against five, and the anonymous constructor has two slots instead of six.',
    ...
    {t:'ex',
     ...
     setup: 'In scope: <code>Hoare</code>, <code>Exec</code> and its <code>load</code> rule, <code>singleton_same</code>, <code>disjoint_empty_left</code>, <code>union_empty_left</code>, <code>Store.set</code>, <code>pure</code>, <code>∗</code>. Two <code>refine</code>s, a <code>show</code> and a <code>simp</code>. The <code>Exec.load</code> rule takes one premise, a proof that the cell is allocated.',
    ...
    {t:'ex',
     ...
     setup: 'In scope: <code>Exec.write</code>, whose premise is a proof that the cell is allocated, and <code>write_singleton (l : Loc) (v w : Val) : Heap.write (Heap.singleton l v) l w = Heap.singleton l w</code>. Three lines after the <code>intro</code>, and no tactic other than <code>exact</code>.',
     ...
     deep: [
       ...
       {t:'p', h:'It is no longer than the real proof, so length is not the objection. What it is, is a proof about the definitions of <code>Heap.write</code> and <code>Heap.singleton</code> rather than about the interface over them — the thing Unit 06 was written to make unnecessary. Change either definition and this proof needs rewriting; <code>write_singleton</code> is a statement about heaps and survives. And a reader following the specification is asked, mid-proof, to decide what happens at an address the program never mentions.'}
     ],
    ...
    {t:'p', h:'Give a command a name and the rule specialises to it without any work. <code>clearCell l</code> is <code>[l] := 0</code>, and its specification is the write rule at <code>e := .const 0</code>: the postcondition <code>fun σ h => (l ↦ (Atom.eval σ (.const 0))) σ h</code> reduces, because <code>Atom.eval</code> on a constant is a pattern match that fires, to <code>l ↦ 0</code>. Lean sees the two as the same type and no rule of consequence is needed.'},
    ...
    {t:'state', cap:'The two obligations left by <code>refine hoare_seq (Q := (l ↦ 1)) ?_ ?_</code>. Both are instances of the write rule; the middle assertion appears once as a postcondition and once as a precondition, which is what makes it a valid cut of the program.',
    ...
    {t:'ex',
     ...
     setup: 'In scope: <code>hoare_seq</code> from Unit 22 and <code>hoare_write</code> from this unit. One term, two applications. If you prefer a tactic proof, <code>refine hoare_seq (Q := (l ↦ 1)) ?_ ?_</code> gives you the two obligations shown above.',
    ...
    {t:'ex',
     ...
     hints: [
       'Unfolded: for every <code>σ</code> and every <code>h = Heap.singleton l v</code>, there is a state <code>s\'</code> with <code>Exec (.load x l ;; .free l) ⟨σ, h⟩ s\'</code>, and at that state <code>σ\' x = v</code> and the heap is empty. Three obligations, not two, because <code>pure</code> is a conjunction.',
