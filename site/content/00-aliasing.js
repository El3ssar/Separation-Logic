registerChapter({
  ...
  /* The promissory note is quoted verbatim from Unit 28 and the page says in so
     ...
     'instance' and 'where' are not used here: they occur inside a compiler
     message quoted verbatim in x01's 'deep', produced by compiling the mistake
     it warns about. Trimming them out would make the quotation a paraphrase. */
  ledgerAllow: ['∀', 'leading-dot name resolution (.ctor)', 'Hoare', 'ptsAtLeast', 'aAnd', 'instance', 'where'],
  ...
  orient: {
    ...
    payoff: 'The problem posed here is the one the next twenty-eight units solve; the theorem Unit 28 refutes is the one written down, unproved, at the bottom of this page.'
  },
  ...
  blocks: [
    ...
    {t:'anat', tag:'verified',
     ...
     parts:[
       {m:'abbrev', h:'Names a type, transparently: Lean unfolds the name whenever it needs to, so <code>Heap</code> and <code>Loc → Option Val</code> are interchangeable everywhere and no step of any proof is spent converting between them. The alternative keyword is <code>def</code>, which would make each of these a type in its own right that happens to be defined that way, and Lean\'s automatic machinery then stops seeing through it. Make these five <code>def</code>s and the very next definition on this page fails to elaborate three times over: the numerals <code>4</code> and <code>5</code> are no longer accepted as a location and a value, and Lean can no longer test <code>x = 4</code>.'},
    ...
    {t:'svg', cap:'The set of addresses where <code>h</code> is <code>some</code> — its domain of definition — is the part to watch. Everything this course does to memory is done to that set.',
     src:'\n<svg viewBox="0 0 560 182" role="img" aria-label="A heap is a partial function from locations to values">\n  <g class="dg">\n    <text x="8" y="18" class="dg-lab">locations (Nat)</text>\n    <text x="404" y="18" class="dg-lab">values (Nat)</text>\n    <g class="dg-dot">\n      <circle cx="60" cy="46" r="5"/><text x="26" y="51" class="dg-t">0</text>\n      <circle cx="60" cy="82" r="5"/><text x="26" y="87" class="dg-t">1</text>\n      <circle cx="60" cy="118" r="5"/><text x="26" y="123" class="dg-t">2</text>\n      <circle cx="60" cy="154" r="5"/><text x="26" y="159" class="dg-t">3</text>\n    </g>\n    <g class="dg-dot">\n      <circle cx="482" cy="70" r="5"/><text x="498" y="75" class="dg-t">7</text>\n      <circle cx="482" cy="142" r="5"/><text x="498" y="147" class="dg-t">4</text>\n    </g>\n    <path class="dg-arr" d="M250 82 C 330 82, 396 70, 476 70"/>\n    <path class="dg-arr" d="M250 154 C 330 154, 396 142, 476 142"/>\n    <text x="82" y="50" class="dg-note">h 0 = none — unallocated</text>\n    <text x="82" y="86" class="dg-note">h 1 = some 7</text>\n    <text x="82" y="122" class="dg-note">h 2 = none — unallocated</text>\n    <text x="82" y="158" class="dg-note">h 3 = some 4</text>\n  </g>\n</svg>'},
    ...
    {t:'p', h:'<code>def</code> introduces a definition: a name, its type after the colon, and after <code>:=</code> the thing the name stands for. <code>fun x => …</code> is Lean\'s lambda — the function sending <code>x</code> to the body. Read the body as a one-row table with a default underneath it: address 4 holds 5, every other address holds nothing.'},

    {t:'code', tag:'illustration', cap:'Two commands that interrogate a definition without proving anything about it, and a third that asks for a proof.',
    ...
    {t:'p', h:'There is one tactic here. <code>simp</code> simplifies, and the bracketed list names the definitions it is allowed to unfold: <code>simp [aliasedAfter]</code> replaces <code>aliasedAfter 4</code> by <code>if 4 = 4 then some 5 else none</code>, settles <code>4 = 4</code>, takes the <code>then</code> branch, and is left with <code>some 5 = some 5</code>, which it closes. Each exercise below carries an editor with a <b>Check with Lean</b> button, and it runs a real Lean kernel on what you typed: success is <i>Lean accepts this proof</i> and a green marker, failure is Lean\'s own message with a line and a column, reproduced without editing. There is a real failure further down the page.'},

    {t:'tbl', cap:'Type the backslash form followed by a space, or tap the glyph in the palette along the top of the editor. The last two are here so your fingers know them before the mathematics arrives.',
    ...
    {t:'ex',
     ...
     setup:'The skeleton above is what you are filling in. The editor starts you at its second half — the <code>example</code>, with a <code>sorry</code> under it — so the definition of <code>twoAllocated</code> is the first thing you type, above the <code>example</code>. The name is fixed by the statement; what it allocates is not entirely: it must allocate cells 4 and 9 and nothing else, and the values it stores there are yours to choose. Then replace the <code>sorry</code>.',
     goal:'example : twoAllocated 7 = none := by',
     hints:[
       'The goal is an equation between two values of type <code>Option Val</code>. On the left, your function applied to 7. On the right, <code>none</code>. Nothing sits above the <code>⊢</code>, so there is no hypothesis to use: everything the proof needs is inside the definition above it.',
       'Your function is a two-row table with a default underneath. Applied to 7 it matches neither row, so it falls through to the default, and the default is “unallocated”. The whole content of the proof is that 7 is neither 4 nor 9 — two arithmetic facts about numerals, and no more.',
       'One tactic: <code>simp</code>, the one that closed <code>aliasedAfter 4 = some 5</code> above. It carries a bracketed list of the definitions it is allowed to unfold, and a definition missing from that list is one it will not touch — so <code>twoAllocated</code> has to be named in the bracket.',
       'The body is <code>fun x =&gt;</code> followed by two nested <code>if</code>s testing 4 and 9, with <code>none</code> in the last <code>else</code>. The proof is the single line <code>simp [twoAllocated]</code>. It unfolds the definition, decides <code>7 = 4</code> and <code>7 = 9</code> to be false, takes the <code>else</code> branch twice, and is left with <code>none = none</code>, which it closes. Nothing remains after it.'
     ],
     sol:'def twoAllocated : Heap := fun x => if x = 4 then some 5 else if x = 9 then some 2 else none\n\nexample : twoAllocated 7 = none := by\n  simp [twoAllocated]',
     solNote:'Any heap named <code>twoAllocated</code> that allocates exactly 4 and 9 is correct, with any values in them and the conditionals in either order. The editor checks your text, not this one.',
     ...
     walk:[
       {tac:'def twoAllocated : Heap := fun x => if x = 4 then some 5 else if x = 9 then some 2 else none',
        h:'Builds the object the goal is about, before there is a goal. <code>fun x =&gt;</code> binds the address being looked up; the nested <code>if</code>s enumerate the two addresses that are allocated; the final <code>else none</code> is what makes this a partial function, because it is the value at every one of the infinitely many addresses the table does not mention. Because <code>Heap</code> is an <code>abbrev</code>, Lean accepts a plain function against that declared type and no conversion step appears anywhere in the proof.'},
     ...
     deep:[
       ...
       {t:'p', h:'The shape is not forced, and that claim is checkable rather than reassuring. Test the addresses in the other order, store different values, and the same lookup falls to the same one-line proof, because the only thing either proof uses is that 7 fails both tests:'},
       {t:'code', tag:'illustration',
        src:'def twoAllocatedAgain : Heap := fun x => if x = 9 then some 0 else if x = 4 then some 0 else none\n\nexample : twoAllocatedAgain 7 = none := by\n  simp [twoAllocatedAgain]'},
       {t:'p', h:'The failure that arrives first is not a failed proof. Write <code>if x = 4 then 5</code>, without the <code>some</code>, and Lean rejects the definition before the <code>example</code> is reached.'},
       {t:'state', cap:'The reply to <code>def twoAllocated : Heap := fun x =&gt; if x = 4 then 5 else …</code>, pointed at the numeral <code>5</code> rather than at the definition. The line-and-column prefix and a trailing <code>Hint:</code> line about tracing the instance search are dropped; nothing else is.',
       ...
       {t:'p', h:'The declared type <code>Heap</code> fixed the codomain to <code>Option Val</code>; the numeral <code>5</code> is not one; and Lean will not insert the <code>some</code> for you. Widening the codomain, three paragraphs into this unit, is the whole reason there is a wrapper here to leave out.'}
     ],
     pitfall:'Writing <code>else some 0</code> for the unallocated case. It type-checks, which is what makes it hard to find, and it is a different heap — it allocates every address in memory and stores 0 in almost all of them — so <code>twoAllocated 7 = none</code> becomes false, and Check answers <code>error: unsolved goals</code> with <code>⊢ False</code> under it, because <code>simp</code> evaluated <code>some 0 = none</code>, decided it, and rewrote the goal to the absurd. The residue names no address, so the only way back is to reread the definition. “Holds nothing” is <code>none</code>; “holds zero” is <code>some 0</code>; keeping those apart is the entire reason for the <code>Option</code>.',
     variants:'Ask instead for <code>twoAllocated 4 = none</code> and the statement is false. Lean prints <code>error: unsolved goals</code> and then <code>⊢ False</code> — not the <code>some 5 = none</code> you might expect, because <code>simp</code> decides that equation rather than parking it, and rewrites the goal it came from to the absurd. Separate that from what happens in <code>x02</code>, where <code>⊢ False</code> is the goal you set out to reach and a contradictory hypothesis discharges it: same display, opposite verdicts, told apart by whether anything above the <code>⊢</code> can kill it. Here nothing can, because there are no hypotheses at all. Second variant: leave <code>twoAllocated</code> out of the bracket and write bare <code>simp</code>. The definition is never unfolded, nothing else in the goal can move, and the answer is <code>error: `simp` made no progress</code> with the goal untouched. That message is worth provoking once, because it marks the difference between a tactic that failed and a tactic that was not given what it needed.'
    },
    ...
    {t:'p', h:'<code>intro</code> is the tactic for an arrow: it moves the antecedent into your hypotheses and leaves the consequent as the goal. Since <code>¬ P</code> <i>is</i> <code>P → False</code>, running <code>intro</code> here assumes the conjunction and leaves <code>False</code> to be proved. The pattern <code>⟨h3, _⟩</code> takes the conjunction apart on the way in: the first component is named <code>h3</code>, and <code>_</code> says the second is not wanted. Running that line leaves the goal shown below. You can look at a goal yourself at any point in any proof: a line reading <code>trace_state</code> makes Lean print the goal as it stands there, and every goal state shown in this course was obtained that way.'},
    ...
    {t:'ex',
     ...
     setup:'You are given the statement and the first tactic; the editor holds both, with a <code>sorry</code> under them. Replace the <code>sorry</code> with one line. <code>aliasedAfter</code> is the one-cell heap defined above.',
     ...
     hints:[
       ...
       'The work happens in the hypothesis, not in the goal. <code>simp</code> can be aimed at a hypothesis, and it then needs two names: the definition it is allowed to unfold, and the hypothesis to aim at. Leave out either one and it does nothing.',
     ...
     deep:[
       ...
       {t:'p', h:'The angle brackets are a convenience, not a requirement. <code>intro h</code> on its own leaves the conjunction whole, as <code>h : aliasedAfter 4 = some 3 ∧ aliasedAfter 4 = some 5</code> above <code>⊢ False</code>, and the second line still finishes: <code>simp</code> works inside a conjunction, so it reduces the first component to <code>False</code>, and a conjunction with a false component is false.'},
       {t:'code', tag:'illustration',
        src:'example : ¬ (aliasedAfter 4 = some 3 ∧ aliasedAfter 4 = some 5) := by\n  intro h\n  simp [aliasedAfter] at h'},
       {t:'p', h:'The dagger on <code>right✝</code> is a prohibition, not decoration. Aim the second line at that hypothesis by typing its displayed name and neither half of the name survives: the identifier is one Lean has no binding for, and <code>✝</code> is not something Lean\'s parser accepts where an identifier belongs, so the two errors come back separately.'},
       {t:'state', cap:'What <code>simp [aliasedAfter] at right✝</code> answers, line-and-column prefixes dropped. Two errors from one word: the identifier, then the dagger after it.',
        src:'error(lean.unknownIdentifier): Unknown identifier `right`\nerror: expected token'}
     ],
     pitfall:'Aiming the tactic at the goal: <code>simp [aliasedAfter]</code> with no <code>at h3</code>. The goal is <code>False</code>, there is nothing in it to unfold or rewrite, and Lean answers <code>error: `simp` made no progress</code> — an error, not a hint, so the proof stops on that line. The same message comes back from the opposite omission, <code>simp at h3</code>, which aims at the right hypothesis and forgets the definition: what <code>h3</code> is made of is <code>aliasedAfter</code>, <code>simp</code> was not allowed to unfold it, and so nothing in <code>h3</code> can move either. One message, two different things left out, and the way to tell them apart is to check both parts before rereading the proof. The content of this exercise is the direction of the work: the contradiction lives in the context, so the tactic has to be pointed at the context — and told what to unfold once it gets there.',
     variants:'Change <code>some 3</code> to <code>some 5</code> and the statement becomes false: the conjunction is then satisfied by <code>aliasedAfter</code>, which is exactly the case where <code>x</code> and <code>y</code> do not alias and the triple is sound. The proof fails in an instructive way rather than a loud one. <code>simp [aliasedAfter] at h3</code> evaluates <code>h3</code> to <code>some 5 = some 5</code>, finds it <i>true</i>, and rewrites <code>h3</code> to <code>True</code> — so nothing closes, and what is left standing is <code>right✝ : aliasedAfter 4 = some 5</code>, <code>h3 : True</code>, <code>⊢ False</code>. A hypothesis that has become <code>True</code> is a hypothesis with no content left in it, and it is the exact point at which the statement stopped being provable. Change the address in the first conjunct from 4 to 7 instead and the same two lines still work, for a different reason: <code>aliasedAfter 7</code> is <code>none</code>, so <code>h3</code> becomes <code>none = some 3</code>, false because the two constructors of <code>Option</code> are distinct rather than because two values differ. Drop the negation and you are asked to <i>prove</i> the conjunction; <code>simp [aliasedAfter]</code> grinds it down to <code>error: unsolved goals</code> with <code>⊢ False</code>, which no tactic can repair, because the claim is not true.'
    },
    ...
    {t:'anat', tag:'illustration',
     ...
     parts:[
       ...
       {m:'(hx : x ≠ 4)', h:'A binder whose type is a proposition — a term of the type Lean calls <code>Prop</code> — so the argument you supply for it is a <i>proof</i>. Hypotheses and arguments are the same thing in Lean; that identification is the subject of Unit 01, and it is why a theorem with a side condition looks like a function with an extra parameter.'},
    ...
    {t:'p', h:'Keyword, name, binders, statement: every declaration from here to the end of the course has that shape, and reading one means finding the colon that closes the binders and taking everything from there to the <code>:=</code> as the claim. In this one <code>hx</code> is not decoration, it is the entire content. Delete it and the claim is false, because <code>x</code> could be 4 — and Lean, asked to prove the deleted version, says something more useful than “no”.'},

    {t:'code', tag:'sketch', cap:'The same theorem with the disequality removed.',
     src:'theorem bad (x : Loc) : aliasedAfter x = none := by\n  simp [aliasedAfter]'},

    {t:'state', cap:'Lean\'s whole reply to the block above, line-and-column prefix dropped.',
     src:'error: unsolved goals\nx : Loc\n⊢ ¬x = 4'},

    {t:'p', h:'Read the residue rather than the word “error”. <code>simp</code> did everything it could and came to rest at <code>⊢ ¬x = 4</code> — which is the hypothesis you deleted, since <code>x ≠ 4</code> and <code>¬ x = 4</code> are the same proposition and Lean prints the second. It has reduced the entire theorem to the one fact it was not given, and is telling you what it is owed. This is the most common shape of failure in the first half of this course — a proof complete except for a claim that two addresses are different — and the habit of reading a leftover goal as a bill rather than as a rejection is worth forming now.'},

    /* ---------------------------------------------------------- the cost --- */

    {t:'h3', s:'What the repair costs'},

    {t:'p', h:'Adding disequality hypotheses works. Classical Hoare logics for languages with pointers do exactly this, and the resulting rules are sound. The damage is not to soundness; it is to specifications. Take a routine that swaps the contents of two cells. Its honest description mentions two addresses. To be usable inside a program that also maintains a linked list, a counter and a buffer, its specification must further say that those two addresses differ from every address the rest of the program is using — and it must say so before anyone knows what the rest of the program is. So the specification of a routine that touches two cells now mentions memory the routine never reads and never writes; it grows with the number of live cells, two disequalities apiece, none of which is about swapping; and it has to be rewritten every time a caller allocates anything new. Across a whole program the same bookkeeping is quadratic — <code>n</code> live cells, pairwise distinct, is on the order of <code>n²</code> conditions to state and to carry. That is the failure, and it is worth being exact about which failure it is. Nothing here is unsound. What is lost is modularity: a specification you cannot write down without knowing the whole program is not a specification.'},
    ...
    {t:'p', h:'<code>x02</code> denied one postcondition about one heap. The rule at the top of the page claims far more: it quantifies over every pair of addresses, every pair of values, and every state the command might run from, and to deny <i>that</i> you need a triple — which needs a language for the command to be written in, a semantics for it to run under, and a definition saying what the triple asserts. None of that exists yet. So the statement goes on the page as it stands, with a name and no proof, and stays there until it can be paid.'},

    {t:'code', tag:'sketch', cap:'Quoted from Unit 28, where it is proved.',
