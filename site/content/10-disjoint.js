registerChapter({
  ...
  blocks: [
    ...
    {t:'svg', cap:'Filled boxes are the domain, and the box in the second colour is the one two heaps both fill. <code>h₁</code> and <code>h₂</code> are disjoint: read down any column and at most one box is filled. <code>h₁</code> and <code>h₃</code> are not, and address 4 is the whole of the reason — everywhere else they behave impeccably.',
    ...
    {t:'trace', title:'The definition opens by itself',
     ...
     steps:[
       ...
       {tac:'exact hd l',
        state:'',
        h:'And the hypothesis is used the same way. <code>hd</code> is a function from addresses to proofs, so <code>hd l</code> is already a proof of that disjunction — no tactic stands between the hypothesis and the fact you want out of it. Every proof on this page that has a disjointness hypothesis spends it like that, and the only decision is which address to spend it at.'}
     ],
     done:'No goals.'},
    ...
    {t:'ex',
     ...
     hints:[
       ...
       'A proof of <code>∀ l, …</code> is a function, so it can be written <code>fun l => …</code> with no <code>by</code> at all. Unit 01 gave the two ways to build a proof of a disjunction — <code>Or.inl</code> and <code>Or.inr</code>, the injections into it — and the one-word term that proves an equation; Unit 02 said exactly when that term is accepted.',
     ...
     expl:'The proof is a function of an address that ignores its argument. At any address the left disjunct is <code>Heap.empty l = none</code>; <code>Heap.empty</code> is <code>fun _ => none</code>, so <code>Heap.empty l</code> reduces to <code>none</code> and both sides of the equation are the same term. That is the condition Unit 02 gave for <code>rfl</code>: nothing here is blocked, because nothing is being cased on. The mirror theorem needs the right injection instead, and the two are genuinely different terms even though the statements are mirror images. It could have been derived rather than written: <code>disjoint_symm</code> is already proved, and <code>disjoint_symm (disjoint_empty_left h)</code> has the right type. The term is shorter and leans on nothing, so that is the one the course keeps.',
     walk:[
       {tac:'fun _ =>', h:'Introduced the address that the <code>∀</code> in <code>Heap.disjoint</code> binds, and threw it away. This is what <code>intro l</code> would have done, written as a term. Naming it <code>_</code> is a statement that the address plays no part. Write <code>fun l =></code> instead and the proof still compiles, over a warning: <code>Variable name `l` is not explicitly referenced. The binding can be removed (if unused) or named `_` (if used implicitly).</code>'},
    ...
    {t:'p', h:'Now the theorem with content, and the debt from the first page of this unit. A specification of a routine is not entitled to assume that its two pointer arguments are different numbers — that is what <code>write_comm</code>\'s hypothesis helped itself to. What a specification <i>can</i> say is that the routine holds the cell at one address and, separately, holds the cell at the other. Two one-cell heaps, disjoint. The claim to prove is that this is enough.'},
    ...
    {t:'trace', title:'What <code>subst</code> removes',
     ...
     steps:[
       {tac:'subst heq',
        state:'h : Heap\nl₁ : Loc\nhn : h l₁ = none\n⊢ h l₁ = none',
        h:'<code>l₂</code> is gone from the context, and so is <code>heq</code> — there is nothing left for the equation to say. Every occurrence of <code>l₂</code> was replaced by <code>l₁</code>, in the goal and in every hypothesis at once. Which of the two survives is decided by the equation and not by you: <code>subst</code> eliminates the variable on the <i>right</i> when there is one, so <code>heq : l₂ = l₁</code> would have removed <code>l₁</code> instead, and <code>heq : l₁ = 4</code>, whose right-hand side is not a variable at all, removes <code>l₁</code>.'},
       {tac:'exact hn', state:'', h:'The hypothesis and the goal are now literally the same statement.'}
     ],
     done:'No goals.'},

    {t:'p', h:'<code>subst heq</code> needs one side of <code>heq</code> to be a local variable that does not occur on the other side. On <code>h l = some v</code> it refuses: <code>invalid equality proof, it is not of the form (x = t) or (t = x)</code>. <code>rw [heq] at hd</code> would also have closed the example above. The difference is bookkeeping: after <code>rw</code> you still have two address variables and a hypothesis relating them, and every later tactic has to name the right one of them. After <code>subst</code> there is one address and no choice left to get wrong. The exercise below is where keeping the choice starts to cost something.'},

    {t:'h4', s:'Two branches that each need two tactics: <code>&lt;;&gt; ·</code>'},

    {t:'p', h:'The second piece is the branching. "Disjointness applied there says one of the two answers <code>none</code>" is a disjunction, split by <code>rcases</code>, whose two branches are the same argument with a different value in it. Here is the concrete case, with the two addresses already equal.'},
    ...
    {t:'ex',
     ...
     solNote:'Two proofs and fifteen lines — the longest exercise so far, and it has four separate decisions in it: which address to split on, which disjunct to take in each branch, which orientation the disequality needs, and where to spend <code>hne</code>. If you have climbed all four hints and it still will not come, open this and read the walkthrough — the shapes here recur for the rest of the course and there is nothing to be gained by grinding.',
     expl:'The first proof has <code>write_comm</code>\'s shape, with a disjunction where that one had an equation. Split on whether the address under consideration is <code>l₁</code>. If it is not, the first heap is undefined there and the left disjunct holds. If it is, the first heap <i>is</i> defined there, so the left disjunct is hopeless — but then the address is <code>l₁</code>, which is not <code>l₂</code>, so the second heap is undefined and the right disjunct holds. That is the only line in the proof that uses <code>hne</code>, and it is the reason the theorem needs it. The second proof\'s <code>mp</code> half is the refutation: assume the addresses are the same, collapse them to one with <code>subst</code>, and read disjointness at that address. Whichever disjunct disjointness offers, it says a one-cell heap is undefined at its own address, and <code>singleton_same</code> plus constructor distinctness kill both. The <code>mpr</code> half is the first theorem with its arguments supplied.',
     walk:[
     ...
     deep:[
       {t:'trace', title:'The first theorem, and where <code>hne</code> is spent',
        start:'l₁ l₂ : Loc\nv₁ v₂ : Val\nhne : l₁ ≠ l₂\n⊢ (Heap.singleton l₁ v₁).disjoint (Heap.singleton l₂ v₂)',
        steps:[
          {tac:'intro x',
           state:'l₁ l₂ : Loc\nv₁ v₂ : Val\nhne : l₁ ≠ l₂\nx : Loc\n⊢ Heap.singleton l₁ v₁ x = none ∨ Heap.singleton l₂ v₂ x = none',
           h:'The definition opened and the goal became a disjunction of two lookups at one address. Neither disjunct is provable yet, because nothing is known about where <code>x</code> sits.'},
          {tac:'by_cases hx : x = l₁',
           state:'case pos\nl₁ l₂ : Loc\nv₁ v₂ : Val\nhne : l₁ ≠ l₂\nx : Loc\nhx : x = l₁\n⊢ Heap.singleton l₁ v₁ x = none ∨ Heap.singleton l₂ v₂ x = none',
           h:'The goal did not move: the split adds a hypothesis and duplicates the goal underneath it. <code>hx</code> is the entire difference between this branch and the one behind it, and it is what decides which disjunct is reachable.'},
          {tac:'right',
           state:'case pos\nl₁ l₂ : Loc\nv₁ v₂ : Val\nhne : l₁ ≠ l₂\nx : Loc\nhx : x = l₁\n⊢ Heap.singleton l₂ v₂ x = none',
           h:'Half the goal is gone, and the half that went was the unprovable one — <code>x</code> is <code>l₁</code>, so the first heap answers <code>some v₁</code> here. The remaining goal names the <i>other</i> address, which is why <code>hne</code> is about to be needed.'},
          {tac:'have : x ≠ l₂ := by rw [hx]; exact hne',
           state:'case pos\nl₁ l₂ : Loc\nv₁ v₂ : Val\nhne : l₁ ≠ l₂\nx : Loc\nhx : x = l₁\nthis : x ≠ l₂\n⊢ Heap.singleton l₂ v₂ x = none',
           h:'One line added to the context and the goal untouched. <code>this</code> is the missing argument of <code>singleton_other</code>, in the orientation that lemma fixes; producing it is the only use <code>hne</code> gets in the whole theorem.'},
          {tac:'exact singleton_other l₂ x v₂ this',
           state:'case neg\nl₁ l₂ : Loc\nv₁ v₂ : Val\nhne : l₁ ≠ l₂\nx : Loc\nhx : ¬x = l₁\n⊢ Heap.singleton l₁ v₁ x = none ∨ Heap.singleton l₂ v₂ x = none',
           h:'The <code>pos</code> branch closed, and what was waiting behind it comes forward: the same disjunction with the hypothesis negated.'},
          {tac:'left',
           state:'case neg\nl₁ l₂ : Loc\nv₁ v₂ : Val\nhne : l₁ ≠ l₂\nx : Loc\nhx : ¬x = l₁\n⊢ Heap.singleton l₁ v₁ x = none',
           h:'The mirror decision, and this time the hypothesis needed is already in the context under the name <code>hx</code>. <code>exact singleton_other l₁ x v₁ hx</code> closes it, and <code>hne</code> is never mentioned — this branch would hold even if the two addresses were the same.'}
        ],
        done:'No goals.'},
    ...
    {t:'ex',
     ...
     deep:[
       ...
       {t:'state', cap:'Both branches survive, in the same state.',
        src:'error: unsolved goals\ncase inl\nh : Heap\nhd : h.disjoint h\nl : Loc\nh1 : h l = none\n⊢ none = Heap.empty l\n\ncase inr\nh : Heap\nhd : h.disjoint h\nl : Loc\nh1 : h l = none\n⊢ none = Heap.empty l'},
       {t:'p', h:'The rewrite worked and the trailing <code>rfl</code> that <code>rw</code> attempts did not fire, although <code>Heap.empty l = none</code> is true by reduction and <code>exact h1</code> exploits exactly that. The two are checked at different settings: <code>rw</code>\'s closing attempt only unfolds definitions marked as cheap to unfold, and <code>Heap.empty</code> is an ordinary <code>def</code>, while <code>exact</code> reduces as far as it needs to. Written as its own tactic, <code>rfl</code> does finish both branches — provided it goes under a dot.'},
       {t:'code', tag:'illustration', cap:'The rewriting version repaired. Written <code>&lt;;&gt; rw [h1]; rfl</code> instead, it leaves <code>case inr</code> standing, exactly as the semicolon did in the main text.',
        src:'theorem x19_rfl (h : Heap) : Heap.disjoint h h → h = Heap.empty := by\n  intro hd\n  funext l\n  rcases hd l with h1 | h1 <;>\n    · rw [h1]\n      rfl'},
       {t:'p', h:'Five tactic lines where the solution\'s forward half is three, and they reach the same place by a route that has to be told twice what <code>exact</code> settles once. That is the argument for <code>exact h1</code>: it is not shorter by luck, it is shorter because it never puts the goal into a state that then needs repairing.'}
     ],
     ...
     variants:'The backward implication on its own is trivial and nothing needs it; the forward one is what Unit 14 spends. Stated with <code>∀ l, h l = none</code> in place of <code>h = Heap.empty</code> it is the same theorem with the <code>funext</code> moved out of the proof and into the statement, and it is weaker in use, because an equation between heaps can be rewritten with and a family of equations cannot. Replace one of the two <code>h</code>s by a different heap and the <code>↔</code> breaks at exactly one point, and it is not the point you might expect. Right to left survives untouched — it is <code>disjoint_empty_left</code>, and an empty heap is disjoint from every heap there is. What goes is left to right: <code>Heap.singleton 4 3</code> and <code>Heap.singleton 9 7</code> are disjoint, by <code>singleton_disjoint</code>, and neither of them is empty. Being disjoint from something <i>else</i> constrains nothing; the entire content of this theorem is that the two arguments are the same heap.'
    },
    ...
    {t:'detail', title:'Disjointness is stronger than it needs to be, and that is the point', open:false,
     blocks:[
       ...
       {t:'p', h:'And compatibility really is enough for the operation to behave: here is <code>Heap.union</code>, proved commutative under it. The case analysis is Unit 09\'s technique, and the proof is here only so that the claim is a theorem rather than an assertion. It is the one thing this page proves about the combination, and nothing later cites it.'},
    ...
    {t:'detail', title:'Why not make the union demand a disjointness proof?', open:false,
     blocks:[
       ...
       {t:'p', h:'Neither side is a heap. Each is still a function waiting for a disjointness proof, and the two proofs wanted are different — one about <code>h₁</code> against a combination, one about a combination against <code>h₃</code>. Neither is available, and producing either is itself a theorem. So the associativity of this operation cannot even be <i>stated</i> until two lemmas about it have been proved, and those lemmas are about the operation whose statement is blocked. That is why <code>Heap.union</code> above asks for no proof: the operation is total, and disjointness travels separately, as a hypothesis at every use site. The cost is one extra hypothesis on almost every lemma in this module, and the next unit is where it starts being paid.'}
     ]},
