registerChapter({
  ...
  blocks: [
    ...
    {t:'ex',
     ...
     pitfall:'Reaching for <code>.symm</code> before the address is chosen, as above. The fix is one application: <code>(hd l).symm</code>, brackets included. Written <code>hd l.symm</code>, Lean parses it as <code>hd (l.symm)</code> and complains about <code>Nat</code> instead.',
     variants:'Drop the <code>.symm</code> and <code>exact hd l</code> reports <code>Type mismatch: hd l has type h₁ l = none ∨ h₂ l = none but is expected to have type h₂ l = none ∨ h₁ l = none</code> — the two heaps are in the wrong order and nothing else is wrong, which is the whole content of the theorem. Stated as an <code>↔</code> instead of an implication it would be no stronger: applying the implication twice gets you back where you started, and the <code>↔</code> would then need unpacking at every use site.'
    },
    ...
    {t:'ex',
     ...
     hints:[
       ...
       'The whole of the first proof is <code>fun _ => Or.inl rfl</code>. The address is never looked at, so its binder is <code>_</code>. The second theorem is the same term with the other injection.'
     ],
     ...
     variants:'<code>Heap.disjoint Heap.empty Heap.empty</code> is proved by either theorem, and by either injection — the empty heap is disjoint from itself, which is a special case of the last exercise on this page. Replace <code>Heap.empty</code> by <code>Heap.singleton l v</code> and both theorems become false, since <code>Heap.singleton l v l</code> is <code>some v</code> and no choice of injection reduces. What survives is the direction of the argument: it is <code>rfl</code> that carries all the weight here, so the theorem is really a statement about how <code>Heap.empty</code> was defined.'
    },
    ...
    {t:'ex',
     ...
     hints:[
       ...
       'First theorem: <code>by_cases</code> on <code>x = l₁</code>, then <code>left</code> or <code>right</code>, then <code>singleton_other</code>, which wants its disequality with the address being looked up written first. In the branch where <code>x</code> <i>is</i> <code>l₁</code> you have to manufacture that disequality; a <code>have</code> whose proof rewrites with the case hypothesis does it. Second theorem: <code>constructor</code>, then <code>subst</code>, <code>rcases</code>, <code>singleton_same</code> and the bullet form of <code>&lt;;&gt;</code>. The <code>mpr</code> half is one <code>exact</code>.',
     ...
     variants:'Drop <code>hne</code> from the first theorem and it is false: the two heaps are then <code>Heap.singleton l v₁</code> and <code>Heap.singleton l v₂</code>, which overlap at <code>l</code>, and the compiled refutation is the concrete case above. It is also false <i>with</i> <code>hne</code> if you weaken it to <code>v₁ ≠ v₂</code>: disjointness never looks at the values, and two different values at the same address are still an overlap. In the other direction, nothing is lost by taking <code>v₁ = v₂</code>; the <code>↔</code> holds for any values at all, which is what makes it a statement about addresses. Replacing <code>subst heq</code> by <code>rw [heq] at hd</code> also compiles, provided the disjointness is then instantiated at <code>l₂</code> rather than <code>l₁</code> — which is the bookkeeping <code>subst</code> exists to remove.'
    },
