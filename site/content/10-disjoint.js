registerChapter({
  ...
  blocks: [
    ...
    {t:'ex',
     ...
     variants:'<code>Heap.disjoint Heap.empty Heap.empty</code> is proved by either theorem, and by either injection — the empty heap is disjoint from itself, which is a special case of the final exercise below. Replace <code>Heap.empty</code> by <code>Heap.singleton l v</code> and both theorems become false, since <code>Heap.singleton l v l</code> is <code>some v</code> and no choice of injection reduces. What survives is the direction of the argument: it is <code>rfl</code> that carries all the weight here, so the theorem is really a statement about how <code>Heap.empty</code> was defined.'
    },
