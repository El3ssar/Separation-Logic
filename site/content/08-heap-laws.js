registerChapter({
  ...
  orient: {
    ...
    payoff: 'Unit 23\'s rules for assignment and deallocation are <code>write_singleton</code> and <code>erase_singleton</code> with a triple wrapped around them, and Unit 25 proves that <code>write</code> is local by running <code>write_comm</code>\'s argument once. Everything pointwise about heaps is finished here.'
  },
  ...
  blocks: [
    ...
    {t:'txt', cap:'One schema, four instances, four theorems. The names on the right are the ones the course uses; three of the four are exercises below.',
    ...
    {t:'p', h:'Each of the four is one instance of a single fact about the then-branch, and each has the same two-line proof. Two more come from the same source with <code>h</code> taken to be <code>Heap.empty</code>: <code>Heap.write Heap.empty l v</code> is <code>Heap.singleton l v</code>, so the first row read at that heap is <code>write_singleton</code> and the second is <code>erase_singleton</code>. What is <i>not</i> an instance of the schema is anything involving two different addresses, which is where the rest of the unit goes.'},
    ...
    {t:'ex',
     ...
     why:'Last write wins. This is the content of the assignment axiom: a specification for <code>[l] := v</code> may promise that <code>l</code> holds <code>v</code> afterwards without saying a word about what <code>l</code> held before, and this equation is why that is sound. It is also the first row of the schema, so proving it is proving the pattern the next three follow.',
       ...
       {t:'trace', title:'The positive branch, one rewrite at a time',
        ...
        steps:[
          {tac:'simp only [Heap.write]',
           ...
           h:'Both writes unfolded and both applications beta-reduced. Three conditionals, one undecided condition, repeated.'},
          {tac:'simp only [hx]',
    ...
    {t:'ex',
     ...
     why:'This is why writing to a cell and then freeing it is the same as freeing it: a write that is undone leaves nothing behind. It is the shape Unit 26 relies on when it shows that locality composes along a sequence — the intermediate heap of a two-command run is not observable in the final one. It also makes a point about <code>simp</code> brackets that is cheaper to learn here than in a proof three hundred lines long.',
    ...
    {t:'ex',
     ...
     why:'Two writes to different cells can be done in either order. That sentence, generalised from two writes to two arbitrary commands and from "different cells" to "disjoint regions of memory", is the frame rule. Unit 25 proves that <code>Heap.write</code> is local, and the argument is this proof with the address arithmetic replaced by a disjointness hypothesis; Unit 27 turns locality into the frame rule. This is the first place the disequality appears in a statement about heaps, and it will not be handed to you again — Unit 08 onwards, you have to derive it.',
     ...
     walk:[
       ...
       {tac:'unfold Heap.write', h:'Replaced all four occurrences by their bodies and beta-reduced, leaving <code>(if x = l₂ then some v₂ else if x = l₁ then some v₁ else h x) = if x = l₁ then some v₁ else if x = l₂ then some v₂ else h x</code>. <code>unfold</code> rather than <code>simp only</code> because nothing is to be simplified — the conditionals are wanted exactly as they stand, so that <code>rw</code> can name them one at a time.'},
     ...
     deep:[
       ...
       {t:'p', h:'So a commutation law needs a disequality exactly when the two operations would put different things at a shared address. Two writes disagree, so <code>write_comm</code> needs one. An erase and a write disagree — <code>none</code> against <code>some v</code> — so the mixed law will need one too. Two erases both put <code>none</code>, so <code>erase_comm</code> needs nothing and is true as it stands. You can predict which of the eleven laws carry a hypothesis without proving any of them.'}
     ],
    ...
    {t:'ex',
     ...
     hints:[
       'The first: at <code>l</code> the left is <code>some w</code> and the right is <code>some w</code>; away from <code>l</code> both are <code>none</code>. The second: erasing the only cell of a one-cell heap. At <code>l</code> the left is <code>none</code> and the right — <code>Heap.empty l</code> — is also <code>none</code>; away from <code>l</code> both are <code>none</code> again, for different reasons.',
       'Both are the schema at <code>h = Heap.empty</code>, so both are three moves and one <code>simp</code> call per branch. The second differs from every proof so far in that its right-hand side is an operation too, and it is the operation with no conditional in it.',
    ...
    {t:'note', kind:'key', title:'Where each of these is spent',
     h:'<code>write_singleton</code> and <code>erase_singleton</code> are Unit 23\'s rules for assignment and deallocation; those proofs are three lines and contain no <code>funext</code>. <code>write_comm</code> is the argument Unit 25 runs to prove that <code>Heap.write</code> is local, with the disequality replaced by a disjointness hypothesis about two whole regions — and locality is what Unit 27 turns into the frame rule. <code>write_of_eq</code> is the shape of every locality proof: a hypothesis saying the heap agrees with something at a point, used at exactly that point. The rest are the equations that make the rewriting in Units 29 and 30 go through without anyone thinking about them. Nothing below this line is proved pointwise again.'},
    ...
    {t:'detail', title:'Why is <code>Heap</code> a function into <code>Option</code>, and not a primitive partial map?', open:false,
     blocks:[
       {t:'p', h:'Because of the eleven proofs on this page. Every one of them is <code>funext</code> followed by a case split followed by a decision about a conditional, and every step of that is available only because a heap is a function and its operations are conditionals. A packaged partial map — a domain predicate together with a value function defined on it — makes every one of these equations into a pair of equations, the second of which does not typecheck until the first is proved, and <code>rw</code> stops finding its patterns because the terms are wrapped in transports. The price of the <code>Option</code> encoding is the ambiguity in <code>none</code> and the loss of <code>rfl</code>; what it buys is that <code>funext</code>, <code>by_cases</code> and <code>if_pos</code> are the entire proof method, on this page and everywhere after it.'}
     ]},
