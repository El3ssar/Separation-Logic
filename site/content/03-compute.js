registerChapter({
  ...
  orient: {
    youWill: [
      ...
      'State the rule that decides when <code>rfl</code> works, and name exactly what blocks it.',
  ...
  blocks: [
    ...
    {t:'note', kind:'key', title:'The rule',
     h:'<code>rfl</code> succeeds when both sides reduce to the same normal form. A definition can always be unfolded, so a <code>def</code> is never what blocks you. What blocks you is a <b>variable in a position that is being cased on</b>: reduction reaches it and has no case to take. When <code>rfl</code> fails on something you believe, look for that variable, and ask which definition was waiting to split on it.'},

    {t:'code', tag:'verified',
     cap:'One more definition, small enough to test the rule against. It joins the course\'s vocabulary and stays in scope for the rest of this page.',
    ...
    {t:'ex',
     ...
     name:'double',
     hard:false,
     why:'Three equations about one three-token definition, of which two are computations and one is not. Telling them apart by eye is the skill; from Unit 05 onwards every heap lemma you meet is one or the other, and reaching for the wrong tactic costs you the goal state you would have learned from. It is also the first place you see <code>simp</code> doing something <code>rfl</code> cannot, which is not a matter of <code>simp</code> being stronger — it is a matter of <code>simp</code> knowing theorems.',
     setup:'<code>def double (n : Nat) : Nat := n + n</code> is in scope. Three separate theorems, and the editor opens with all three statements and a <code>sorry</code> under each; replace the three <code>sorry</code>s. The first two want a term, the third wants one tactic under the <code>by</code> that is already there. Nothing beyond <code>rfl</code> and <code>simp</code> is needed.',
     goal:'theorem double_unfold (n : Nat) : double n = n + n :=\n  sorry\n\ntheorem double_three : double 3 = 6 :=\n  sorry\n\ntheorem double_zero_left (n : Nat) : double n = 0 + n + n := by',
     hints:[
       'The first goal says that <code>double</code> applied to an arbitrary <code>n</code> is <code>n + n</code>. The second says <code>double 3</code> is <code>6</code>. The third says <code>double n</code> is <code>0 + n + n</code>, which is <code>(0 + n) + n</code> — the bracketing matters, and so does which side of the <code>+</code> the <code>0</code> is on.',
       'Ask of each: can Lean get from one side to the other by unfolding and computing alone? Unfolding <code>double</code> is always allowed. Computing <code>3 + 3</code> is allowed, because both arguments are numerals. Computing <code>0 + n</code> is not, because the second argument is a variable and addition splits on it — but <code>0 + n = n</code> is still a theorem, and somebody has already proved it.',
       'The first two want <code>rfl</code>, written as a term: <code>:=</code> and then the word, with no <code>by</code>. The third wants <code>simp</code>, and <code>simp</code> has to be told that <code>double</code> is one of the equations it may use — the bracketed form from Unit 00 is what tells it. The theorem that clears the <code>0 +</code> is <code>Nat.zero_add</code>, and that one is already in <code>simp</code>\'s default stock, so it does not go in the bracket.',
       'The first is <code>:= rfl</code>: unfolding <code>double n</code> gives <code>n + n</code> on the nose. The second is <code>:= rfl</code> as well. The third is <code>by</code> and then <code>simp [double]</code> — the bracket adds the defining equation of <code>double</code> to <code>simp</code>\'s stock, and the theorem that clears the <code>0 +</code> is already in it.'
     ],
     sol:'theorem double_unfold (n : Nat) : double n = n + n := rfl\n\ntheorem double_three : double 3 = 6 := rfl\n\ntheorem double_zero_left (n : Nat) : double n = 0 + n + n := by\n  simp [double]',
     ...
     expl:'<code>double_unfold</code> holds because unfolding a <code>def</code> is part of reduction; there is no computation in it at all. <code>double_three</code> holds because both sides reduce to the same numeral. <code>double_zero_left</code> fails that test on account of <code>0 + n</code> and nothing else, and <code>simp</code> closes it not by computing harder but by applying a theorem that was proved by induction long before you got here.',
     walk:[
       {tac:'double_unfold := rfl', h:'The goal is <code>double n = n + n</code>. Reduction unfolds <code>double</code> on the left and stops, because <code>n + n</code> can go no further with <code>n</code> a variable. The right-hand side is already <code>n + n</code>. Same term, so <code>rfl</code> typechecks.'},
       {tac:'double_three := rfl', h:'Unfolding gives <code>3 + 3</code>, and now both arguments of <code>+</code> are numerals, so the case split has something to split on and runs to the end. Both sides become the same numeral. Nothing here differs in kind from the previous line; it only does more work.'},
       {tac:'simp [double]', h:'Turned <code>double n = 0 + n + n</code> into <code>n + n = 0 + n + n</code> by the bracketed defining equation, then cleared the <code>0 +</code> by <code>Nat.zero_add</code>, which is in <code>simp</code>\'s default stock, leaving two identical sides and no goal. Without the bracket <code>simp</code> would not have touched <code>double</code>: a <code>def</code> is not a rewrite rule until you say so.'}
     ],
     deep:[
       {t:'trace', title:'The third one, split in two so you can see which half is which',
        start:'n : Nat\n⊢ double n = 0 + n + n',
        steps:[
       ...
       {t:'p', h:'And the failure the first two theorems set you up for — the third one written as a term, the way they are:'},
       {t:'code', tag:'sketch', src:'theorem double_zero_left (n : Nat) : double n = 0 + n + n := rfl'},
       {t:'state', cap:'Two errors from one line. The first is the real one; the second is what is left over once the first has failed, and it says nothing about your goal.',
        src:'error: Not a definitional equality: the left-hand side\n  double n\nis not definitionally equal to the right-hand side\n  0 + n + n\n\nerror: Type mismatch\n  rfl\nhas type\n  ?m.13 = ?m.13\nbut is expected to have type\n  double n = 0 + n + n'}
     ],
     pitfall:'Reaching for <code>rfl</code> on the third one because the first two took it, and reading the refusal as "Lean cannot see that <code>0 + n</code> is <code>n</code>". Lean can prove that and is not being asked to: <code>rfl</code> is a request for a syntactic outcome after reduction, and reduction of <code>0 + n</code> stops at the variable. What makes the message hard to use is printed above. It gives you the two sides <i>as you wrote them</i> — <code>double n</code> against <code>0 + n + n</code> — and not the normal forms it actually compared, so the subterm that blocked it is never pointed at. Finding it is your job, and the rule above says where to look: the variable sitting where a definition wants to take cases.',
     variants:'Change the third statement to <code>double n = n + 0 + n</code> and <code>rfl</code> closes it, because <code>n + 0</code> reduces and <code>0 + n</code> does not — the same two symbols in the other order. Change it to <code>double n = n * 2</code> and something stranger happens: <code>rfl</code> fails, and <code>simp [double]</code> fails too, leaving <code>n + n = n * 2</code>. Yet <code>n * 2</code> and <code>0 + n + n</code> are the <i>same term</i>. Multiplication recurses on its second argument exactly as addition does, so <code>n * 2</code> unrolls to <code>n * 1 + n</code>, to <code>(n * 0 + n) + n</code>, to <code>(0 + n) + n</code> — and <code>example (n : Nat) : n * 2 = 0 + n + n := rfl</code> is accepted. The third statement and this variant are therefore definitionally the same statement, and <code>simp [double]</code> closes one of them and not the other, because <code>simp</code> rewrites the syntax in front of it and never reduces to find out what that syntax means. Drop the bracket from <code>simp [double]</code> and the goal comes back as <code>double n = n + n</code>, with the <code>0 +</code> cleared and the definition untouched — the clearest single demonstration on this page that unfolding and rewriting are two different operations.'
    },
    ...
    {t:'code', tag:'illustration',
     cap:'Injectivity backwards, with no <code>Option</code> lemma in sight; then a false hypothesis closing a goal that has nothing to do with it. The <code>by simp</code> in argument position supplies the <code>¬ (some v = none)</code>.',
     src:'example (v w : Val) (h : v = w) : some v = some w := congrArg some h\n\nexample (v : Val) (h : some v = none) : 2 + 2 = 5 := absurd h (by simp)'},
    ...
    {t:'code', tag:'verified',
     cap:'From Unit 00, and now readable. <code>Loc := Nat</code> is what makes the conditional in every heap literal legal; <code>Var := Nat</code> is the same decision taken a second time, for the same reason, since a store literal tests a variable the way a heap literal tests an address.',
    ...
    {t:'detail', title:'What decidability costs, and what open Classical would buy', tag:'aside', open:false,
     blocks:[
       ...
       {t:'state', cap:'The price, stated by Lean in one word.',
        src:'error(lean.dependsOnNoncomputable): failed to compile definition, consider marking it as \'noncomputable\' because it depends on \'propDecidable\', which is \'noncomputable\''},
