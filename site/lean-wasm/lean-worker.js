...
self.onmessage = (event) => {
  ...
  } else if (msg.type === 'load_snapshot_bytes') {
    // The streaming path above builds the MEMFS file with incremental FS.write
    // calls; that turned out not to produce a region this build will accept.
    // Writing the whole buffer in one FS.writeFile does work — it is exactly
    // what tools/lean-harness.cjs does, and that is verified. The caller hands
    // us the bytes as a transferable, so there is still only one copy.
    self.postMessage({ type: 'snapshot_loaded', ...loadSnapshotBytes(msg.data) });
  }
};
...
function loadSnapshotBytes(buf) {
  if (!moduleReady) return { success: false, error: 'Module not ready' };
  if (!Module._lean_wasm_load_snapshot) return { success: false, error: 'this build has no snapshot support' };
  const t0 = performance.now();
  const FS = Module.FS;
  const p = '/snapshots/init.snap';
  try {
    const bytes = new Uint8Array(buf);
    if (bytes.length < 5 || bytes[0] !== 0x6f || bytes[1] !== 0x6c || bytes[2] !== 0x65 || bytes[3] !== 0x61 || bytes[4] !== 0x6e) {
      return { success: false, error: 'not an olean region (wrong magic)' };
    }
    ...
    const elapsed = performance.now() - t0;
    console.log(`[SNAPSHOT] direct load ${tag === 0 && ret === 0 ? 'ok' : 'FAILED'} in ${elapsed.toFixed(0)}ms (tag ${tag}, ret ${ret})`);
    return { success: tag === 0 && ret === 0, elapsed, tag, ret };
  } catch (e) {
